...
1. 阅读当前 GitHub Issue 的完整描述、评论、验收标准和允许修改范围；
2. 阅读本文件及与任务相关的需求、架构、数据库和接口文档；
3. 阅读目标模块现有代码、测试和最近相关迁移；
4. 检查当前分支、`git status` 和相对 `main` 的差异；
5. 确认公开 Schema、状态枚举、数据库表、Service Protocol 和 Agent Tool 是否会变化；
6. 涉及 Agent 时，确认当前 Tool Registry、Runner 动态工具白名单、Skill 和模型适配器；
7. 涉及数据库时，确认当前 Alembic head 和迁移依赖链；
8. 涉及写操作时，确认身份、权限、状态、版本、幂等和审计边界。

第一轮应输出：

* 对需求的理解；
* 计划修改的文件；
* API、数据库、Agent Tool 和公共契约影响；
* 实施步骤；
* 测试计划；
* 已知风险和不在本次范围内的事项。

除非 Issue 已经明确授权直接实施，否则计划获得人工确认后再开始修改。

不得重复询问 Issue、文档或当前对话中已经明确给出的信息。

## 3. 范围和协作

* 只修改 Issue 明确允许的文件和模块。
* 需要越过允许范围时，先说明必要性、影响和替代方案。
* 不得借任务之名进行无关重构、依赖升级、目录迁移或格式化整个仓库。
* A 主责工程底座、公共能力、身份权限、Agent、会话和采购需求基础。
* B 主责产品、供应商、白名单、黑名单、推荐、审批、采购、交付、验收和入库。
* `docs/`、`migrations/`、`pyproject.toml`、部署配置、公共 Schema 和本文件属于共同审查范围。
* 跨模块并行开发前，先确定公开 Service Protocol、Pydantic Schema、错误码和状态语义。
* 不允许一个模块直接引用另一个模块的 Repository 或 ORM Model。
* 不得同时维护两个功能重复但行为不同的 Agent 主循环。
