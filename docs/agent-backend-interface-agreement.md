...
- 金额和精确数量：JSON 字符串，例如 `"2.0000"`、`"35000.00"`，禁止 JSON 浮点数。
...
| `category_name` | string/null | 可选 | 必填 | 申请类别快照 |
...
| `product_full_name` | string/null | 可选 | 条件必填 | 具体设备全称；品类模板要求时必填 |
| `brand` | string/null | 可选 | 条件必填 | 品牌；未知或不适用时允许为空并提示确认 |
| `model` | string/null | 可选 | 条件必填 | 型号；未知或不适用时允许为空并提示确认 |
| `specification` | string/null | 可选 | 条件必填 | 规格参数；由品类模板决定 |
| `quantity` | decimal string/null | 可选 | 必填 | 必须大于 0 |
| `unit` | string/null | 可选 | 必填 | 台、个、套等 |
| `supplier_id` | integer/null | 可选 | 可选 | 匹配到的正式供应商 ID |
| `supplier_name` | string/null | 可选 | 可选 | 员工当前选择的供应商名称快照 |
| `unit_price` | decimal string/null | 可选 | 可选 | 员工已知的预算/参考单价，不等同采购报价 |
...
- 如果员工尚未选择供应商，`supplier_id` 和 `supplier_name` 均可为空，不妨碍创建草稿；是否允许正式提交由当前品类/流程校验结果决定。
...
| --- | --- | --- | --- |
| Planned | `POST /api/v1/purchase-requirements/drafts` | 创建采购草稿 | 首次识别到采购意图并取得至少一个有效业务字段后调用一次 |
| Planned | `GET /api/v1/purchase-requirements/{id}` | 获取完整采购单 | 恢复会话、展示确认页和提交成功后刷新状态时调用 |
| Planned | `PATCH /api/v1/purchase-requirements/{id}` | 增量修改草稿 | 每轮补充或员工纠正字段后调用；不要重复创建草稿 |
| Planned | `POST /api/v1/purchase-requirements/{id}/submit` | 员工确认后提交审批 | 仅在明确人工确认后调用 |
| Planned | `POST /api/v1/purchase-requirements/{id}/cancel` | 取消未审批草稿 | 员工明确取消时调用 |
| Planned | `GET /api/v1/purchase-requirements` | 查询本人采购申请 | 状态查询或找回历史草稿时调用，必须分页 |
| Planned | `POST /api/v1/recommendations/historical-suppliers/search` | 查询相似历史采购和供应商 | 商品信息足以检索时调用；结果仅供参考 |
