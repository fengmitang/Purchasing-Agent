...
当前所有采购意图统一进入 GeneralAgent，由命中的 Skill 提供业务指导。
权限、名单校验、状态流转和数据写入必须由后端接口执行。
"""
...
from app.modules.agent.intent_recognizer import IntentCategory, IntentRecognizer, UrgencyLevel
...
class Request:
    ...
    history: list[dict[str, str]] | None = None  # 对话历史，传给意图识别
...
class BaseAgent:
    ...
    def __init__(self, client: Any, model: str, skill_manager: Any | None = None):
        self._client = client
        self._model = model
    ...
    def _needs_escalation(self, content: str) -> bool:
        ...
        keywords = ["转人工", "人工确认", "escalate", "specialist", "无法处理"]
...
class GeneralAgent(BaseAgent):
    ...
    system_prompt = (
        "你是数据中心采购流程助手。负责理解采购需求、整理草稿、解释历史供应商推荐，"
        "并在提交前要求员工明确确认。后端是采购状态、权限和业务事实的唯一来源；"
        "不得编造采购记录、供应商、价格、单号或处理状态。"
    )
...
class AgentOrchestrator:
    ...
    # 当前采购意图统一交给通用执行器；后续流程由显式 Handler/Service 接管。
    ...
    def __init__(
        ...
        # 单一通用执行器负责非采购兜底；采购流程由 AgentService 显式处理。
        self._pool: dict[AgentType, list[BaseAgent]] = {
            AgentType.GENERAL: [GeneralAgent(client, model, skill_manager)],
        }

    def set_skill_manager(self, skill_manager: Any | None) -> None:
    ...
    async def run(self, req: Request) -> OrchestratorResult:
        ...
        # Procurement drafts use the explicit AgentService flow.
        if (
            self._agent_service is not None
            and req.actor is not None
            and req.intent is not None
            and self._agent_service.should_handle(
                req.intent,
                req.user_id,
                req.conv_id,
                history=req.history,
            ...
            agent_result = await self._agent_service.handle(
                intent=req.intent,
                message=req.message,
                user_id=req.user_id,
                conv_id=req.conv_id,
            ...
            req.intent = agent_result.intent
    ...
    def _route(self, intent: IntentCategory | None, urgency: UrgencyLevel | None) -> AgentType:
        """
        三层路由决策：
          1. 意图映射
