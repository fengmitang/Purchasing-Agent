...
- MVP 采用模块化单体和自研受控 Tool Calling Agent，不使用 LangGraph、AutoGen、CrewAI 等 Agent 编排框架。

模型根据当前目标、对话上下文、会话状态和已注册工具集合决定下一步；
...
加密码登录、通过 Agent 对话自动收集采购信息并生成草稿、在结构化摘要中核对后提交审批，也支持
手动填写采购申请、保存或修改草稿、查询历史供应商、取消草稿和查看本人申请。
Agent 当前也可在动态工具白名单允许时提交完整草稿、确认取消本人草稿、查询当前草稿的历史
供应商推荐，以及分页列出本人采购申请；推荐结果不会自动改写草稿，列表结果不会自动切换活动草稿。
登录状态由后端服务端会话和安全 Cookie 保存，页面不在浏览器本地保存密码或登录令牌。楼长可以
查看职责楼宇内的待审批申请并通过或驳回；采购员可以领取审批通过的任务，记录询价、合同和验收入库状态。
...
启动 API 后，可以使用正式 Cookie 会话运行交互式 Agent CLI：

```powershell
.venv\Scripts\python -m app.cli.chat --identifier DEV-E0001
```

CLI 会安全提示输入密码，登录 `/api/v1/auth/login` 后自动保存并携带 HttpOnly Session Cookie，因此 Agent 能获得数据库中的员工、组织和楼宇权限。自动化环境可临时使用 `PROCUREMENT_AGENT_CLI_PASSWORD`，不得提交该变量或输出密码。`--dev-headers` 只用于本地兼容调试，不会加载数据库楼宇范围，不能用于验证采购写操作。
...
当前 Agent 采用单一受控 Tool Calling 主循环：
...
- `RouteResolver` 只决定进入 `GENERAL` 或 `PROCUREMENT` 领域。
- `AgentLoopRuntime` 负责唯一的模型—工具循环；旧 `ProcurementAgentRunner` 名称和兼容别名已删除。
- Procurement 场景向模型提供全部已注册采购 Tool Schema，General 场景不提供采购工具。
- Procurement 路由默认允许全部已注册工具；`ToolPolicy` 只前置限制三类高风险写操作：提交须明确确认，取消须明确确认并提供原因，更新须存在活动草稿。
- ToolExecutor 对每次调用再次校验 `allowed_tools`，未授权的可见工具返回 `TOOL_NOT_ALLOWED`。
- `MemoryProvider` 和 `KnowledgeProvider` 当前仅提供只读接口预留，不参与权限、状态和正式事实判断。
- 工作记忆、知识库、工作单 Agent 尚未作为可执行业务能力开放。

当前状态边界为：ChatStore 保存聊天、锁、幂等和 `active_route`；ProcurementSessionStore 保存活动采购草稿短期状态；MySQL 保存正式采购事实。`AgentDefinition` 和 `PromptProvider` 已接入统一 Agent Runtime，遗留 `orchestrator.py` 已删除。

配置 Redis 时，ChatStore 与 ProcurementSessionStore 均使用异步 Redis，并以 `organization_id + user_id + conversation_id` 隔离状态；内存 Store 仅用于本地和测试。采购细粒度 Intent 统一由 `ModelProcurementIntentResolver` 处理，旧 `IntentRecognizer` 已删除，`IntentCategory` 归入 `enums.py`。

`AgentChatService` 先执行领域路由，只有 `PROCUREMENT` 才调用采购 Intent Resolver；不确定的新会话进入无工具安全追问。`ResponseGuard` 会阻止没有后端成功结果时的虚假提交/取消声明。Memory/Knowledge 当前支持只读召回和失败降级，不参与工具授权或正式业务事实判断。

当前 Bootstrap 已装配 General/Procurement 两个 `AgentDefinition`，采购 Skill 通过 `SkillManagerSelector` 注入；`AgentService.should_handle` 已删除，领域路由唯一入口为 `RouteResolver`。

采购业务 Prompt 已物理迁移到 `app/modules/agent/procurement/prompt.py`，由 `ProcurementPromptProvider` 独立构建；统一 Runtime 只调用 `AgentDefinition.prompt_provider`，不再保存采购规则文本或 Skill 拼接逻辑。

会话重置会同时清理 ChatStore 中的历史、路由、聊天幂等状态和 ProcurementSessionStore 中的短期活动草稿引用；不会删除、取消或修改 MySQL 中的采购事实。

聊天回合采用提交式持久化：用户和助手消息先保持 `PROCESSING`，Route、采购 Session 和幂等结果全部保存后才转为 `COMPLETED`；任一步失败会将本轮两条消息标记为 `FAILED`，并恢复此前的 Route 与采购 Session。跨 Store 重置是幂等操作，局部失败后使用相同请求重试会最终清理两个 Store。CI 通过真实 Redis Service 验证 TTL、序列化、租户隔离和清理行为。
